"""Parser for VGG CNN S caffe model."""
...
VGGCNN_S_PATH = get_dataset_dir("caffe/vgg_cnn")


def fetch_vggcnn_s_protobuffer_file(caffemodel_file=None):
    """Checks for existence of caffemodel protobuffer.
    Downloads it if it cannot be found."""

    default_filename = os.path.join(VGGCNN_S_PATH,
                                    "VGG_CNN_S.caffemodel.caffemodel")
    ...
    if caffemodel_file is None:
        caffemodel_file = default_filename
        if not os.path.exists(VGGCNN_S_PATH):
            os.makedirs(VGG_CNN_S)
...
def fetch_vggcnn_s_architecture(caffemodel_parsed=None,
                                 caffemodel_protobuffer=None):
    """Fetch a pickled version of the caffe model, represented as list of
    dictionaries."""

    default_filename = os.path.join(VGGCNN_S_PATH, 'VGG_CNN_S.pickle')
    ...
    protobuf_file = fetch_vggcnn_s_protobuffer_file(caffemodel_protobuffer)
...
def create_theano_expressions(model=None, verbose=0):
    ...
    if model is None:
        model = fetch_vggcnn_s_architecture()
...
def _get_fprop(output_layers=('fc8',), model=None, verbose=0):
    ...
    if model is None:
        model = fetch_vggcnn_s_architecture(model)
...
class VGGCNNSTransformer(BaseEstimator, TransformerMixin):
    """
    A transformer/feature extractor for images using VGG CNN S.

    Parameters
    ----------
    output_layers : iterable, optional (default=('fc8',))
        Which layers to return. Can be used to retrieve multiple levels of
        output with a single call to transform.

    force_reshape : boolean, optional (default=False)
    ...
    def __init__(self, output_layers=('fc8',),
                 force_reshape=False, transpose_order=(0, 3, 1, 2)):
...
class VGGCNNSClassifier(BaseEstimator):
    """
    A classifier for cropped images using the OverFeat neural network class labels.
    ...
    def __init__(self, top_n=5, large_network=False, output_strings=True,
        ...
        self.transpose_order = transpose_order
        self.transform_function = _get_fprop(('prob',))
